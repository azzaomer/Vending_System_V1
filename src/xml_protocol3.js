...
// P1.4: XML Protocol Gateway & Communication Engine
const axios = require('axios');
// Fix: Destructure required components (Builder and parser) directly from xml2js for maximum robustness
...
// FIX: Instantiate using the destructured 'Builder' class, not 'xml2js.Builder'
const XML_BUILDER = new Builder({ rootName: 'xml', headless: true, renderOpts: { pretty: false } });
...
function buildXmlRequest(action, params) {
    // Instantiate the builder here to ensure it is always ready to use
...
async function sendRequest(action, xmlRequest) {
    if (SHOULD_MOCK_HUB) {
        ...
        // Use the same XML parser as the real response path
        const parsedJson = await xml2js.parseStringPromise(mockXmlResponse, { explicitArray: false, attrkey: '$' });
        return parsedJson;
    }
